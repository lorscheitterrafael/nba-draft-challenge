...
function renderPlayerList(players) {
  ...
  players.forEach(player => {
    const alreadyPicked = state.usedPlayerIds.includes(player.id) ||
                          state.usedPlayerNames.includes(player.name.toLowerCase());
...
function selectPlayer(player, position) {
  ...
  state.usedPlayerNames.push(player.name.toLowerCase());
...
// Animação quarto a quarto — reutilizada pelo modo solo e multiplayer
function runGameAnimation(userFinal, aiFinal, oppName, onDone) {
  ...
  $('live-score-opp-name').textContent = oppName;
...
// Simula um jogo da série atual — modo solo
function simOneGame() {
  if (state.mpMode) { mpSimOneGame(); return; }
  ...
  runGameAnimation(userFinal, aiFinal, opp.name, () => finishGame(userWins, userFinal, aiFinal));
}
...
function seriesResult() {
  if (state.mpMode) { mpSeriesResult(); return; }
...
// Estado do bracket MP (usado durante as séries animadas)
let mpBracket = { rounds: [], roundIndex: 0, gameIndex: 0, entries: [] };
...
function mpRenderLobby(room) {
  ...
  $('mp-lobby-hint').textContent = players.length < 2
...
function mpStartDraft() {
  ...
  MP.listenUsedPlayers(ids => {
    ...
    // Deriva nomes dos IDs para bloquear mesmo jogador em times diferentes
    state.usedPlayerNames = [];
    ids.forEach(id => {
      NBA_TEAMS.forEach(team => {
        const p = team.players.find(pl => pl.id === id);
        if (p) state.usedPlayerNames.push(p.name.toLowerCase());
      });
    });
...
// ---- Waiting (todos terminam o draft) ----
...
function mpRenderWaiting(room) {
  ...
  // Bracket já gerado pelo host → inicia séries
  if (room.bracket) {
    mpStartBracket(room);
    return;
  }

  // Todos prontos e sou o host → gero o bracket
  const allDone = Object.values(room.players || {}).every(p => p.draftDone);
  if (allDone && MP.isHost) mpGenerateAndSaveBracket(room);
}

// ---- Bracket: geração (host) ----
...
// Gera todos os jogos de uma série (pré-determinados) e retorna o objeto da rodada
function generateRoundData(eA, eB) {
  const games = [];
  let wA = 0, wB = 0;
  while (wA < 4 && wB < 4) {
    const aWins = simGame(eA.avg, eB.avg);
    const { winner: ws, loser: ls } = generateScore(
      aWins ? eA.avg : eB.avg, aWins ? eB.avg : eA.avg
    );
    games.push({ aWins, aScore: aWins ? ws : ls, bScore: aWins ? ls : ws });
    aWins ? wA++ : wB++;
  }
  const winnerPid = wA === 4 ? eA.pid : eB.pid;
  return { pidA: eA.pid, nameA: eA.name, avgA: eA.avg,
           pidB: eB.pid, nameB: eB.name, avgB: eB.avg,
           lineupA: eA.lineup, lineupB: eB.lineup,
           games, winnerPid };
}

async function mpGenerateAndSaveBracket(room) {
  ...
  if (entries.length === 2) {
    rounds.push(generateRoundData(entries[0], entries[1]));
  } else if (entries.length === 3) {
    const semi  = generateRoundData(entries[1], entries[2]);
    const finB  = entries.find(e => e.pid === semi.winnerPid);
    const final = generateRoundData(entries[0], finB);
    rounds.push(semi, final);
  } else {
    const s1    = generateRoundData(entries[0], entries[3]);
    const s2    = generateRoundData(entries[1], entries[2]);
    const winS1 = entries.find(e => e.pid === s1.winnerPid);
    const winS2 = entries.find(e => e.pid === s2.winnerPid);
    const final = generateRoundData(winS1, winS2);
    rounds.push(s1, s2, final);
  }

  const championPid = rounds[rounds.length - 1].winnerPid;
  await db.ref(`rooms/${MP.code}/bracket`).set({ rounds, championPid });
}

// ---- Bracket: execução (todos os clientes) ----

let _bracketStarted = false;

function mpStartBracket(room) {
  if (_bracketStarted) return;
  _bracketStarted = true;

  const { rounds, championPid } = room.bracket;
  const entries = Object.entries(room.players || {})
    .map(([pid, p]) => ({ pid, name: p.name, lineup: p.lineup,
                          avg: calcLineupAvgFromObj(p.lineup) }))
    .sort((a, b) => b.avg - a.avg);

  mpBracket.rounds    = rounds;
  mpBracket.entries   = entries;
  mpBracket.roundIndex = 0;
  mpBracket.gameIndex  = 0;
  mpBracket.championPid = championPid;

  mpPlayMyRound();
}

function mpPlayMyRound() {
  const rounds = mpBracket.rounds;
  const idx    = mpBracket.roundIndex;

  if (idx >= rounds.length) {
    // Sem mais rodadas com minha participação → vai direto pros resultados
    mpShowFinalResults();
    return;
  }

  const round = rounds[idx];
  const iAmInThisRound = (round.pidA === MP.pid || round.pidB === MP.pid);

  if (!iAmInThisRound) {
    // Essa rodada não é minha (bracket de 3-4 jogadores): pula para a próxima
    mpBracket.roundIndex++;
    mpBracket.gameIndex = 0;
    mpPlayMyRound();
    return;
  }

  // Monta oponente como se fosse um time da série
  const iAmA   = round.pidA === MP.pid;
  const oppName = iAmA ? round.nameB : round.nameA;
  const oppAvg  = iAmA ? round.avgB  : round.avgA;
  const oppLineup = iAmA ? round.lineupB : round.lineupA;
  const myAvg   = iAmA ? round.avgA  : round.avgB;

  const roundLabel = mpBracket.rounds.length === 1 ? 'Final'
    : idx < mpBracket.rounds.length - 1 ? 'Semifinal' : 'Final';

  // Configura state.playoffs para reutilizar a UI de série
  state.playoffs.round    = 1;
  state.playoffs.userAvg  = myAvg;
  state.playoffs.series   = { u: 0, ai: 0 };
  state.playoffs.gameNum  = 0;
  state.playoffs.opponents = [{
    id: iAmA ? round.pidB : round.pidA,
    name: oppName,
    season: `OVR ${oppAvg}`,
    players: Object.values(oppLineup || {})
  }];

  mpBracket.gameIndex = 0;

  // Monta a tela de série
  $('series-round-badge').textContent = roundLabel;
  $('series-u-wins').textContent  = '0';
  $('series-ai-wins').textContent = '0';
  $('series-opp-name').textContent = oppName;
  $('series-log').innerHTML = '';
  $('btn-sim-game').classList.remove('hidden');
  $('btn-sim-game').disabled = false;
  $('btn-series-next').classList.add('hidden');

  renderSeriesRoster('series-user-list', Object.values(state.lineup), myAvg, 'series-user-avg');
  $('series-opp-title').textContent = oppName;
  renderSeriesRoster('series-opp-list', Object.values(oppLineup || {}), oppAvg, 'series-opp-avg');

  goTo('series');
}

// Chamado quando o jogador clica "Simular Próximo Jogo" em modo MP
function mpSimOneGame() {
  const round = mpBracket.rounds[mpBracket.roundIndex];
  const game  = round.games[mpBracket.gameIndex++];

  const iAmA     = round.pidA === MP.pid;
  const userWins = iAmA ? game.aWins  : !game.aWins;
  const userScore = iAmA ? game.aScore : game.bScore;
  const aiScore   = iAmA ? game.bScore : game.aScore;
  const oppName   = iAmA ? round.nameB : round.nameA;

  state.playoffs.gameNum++;
  runGameAnimation(userScore, aiScore, oppName, () => finishGame(userWins, userScore, aiScore));
}

// Chamado após série MP terminar
function mpSeriesResult() {
  const iWon = state.playoffs.series.u === 4;

  if (!iWon) {
    // Eliminado — pula rodadas restantes e mostra resultado
    mpShowFinalResults();
    return;
  }

  // Avança para próxima rodada que me inclui
  mpBracket.roundIndex++;
  mpBracket.gameIndex = 0;
  mpPlayMyRound();
}
...
function mpShowFinalResults() {
  const entries   = mpBracket.entries;
  const champPid  = mpBracket.championPid;
  const champion  = entries.find(e => e.pid === champPid) || entries[0];
  ...
  div.innerHTML = entries.map(p => `
    <div class="mp-result-team ${p.pid === champPid ? 'is-champion' : ''}">
      <div class="mp-result-team-header">
        ${p.pid === champPid ? '<span class="mp-result-crown">🏆</span>' : ''}

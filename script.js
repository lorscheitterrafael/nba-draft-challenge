...
function renderPlayerList(players) {
  ...
  players.forEach(player => {
    ...
    li.className = 'player-list-item' + (unavailable ? ' unavailable' : '');
    ...
    // Botões do picker (só gerados se não indisponível)
    const pickerBtns = available.map(pos =>
      `<button class="pos-btn" data-pos="${pos}">
        <strong>${pos}</strong>
        <small>${POS_NAMES[pos]}</small>
      </button>`
    ).join('');
...
function selectPlayer(player, position) {
  ...
  hideMessage();
  // Armazena com a posição atribuída (pode ser diferente da natural)
